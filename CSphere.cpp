...
/*---------------------------------------------------------------------------*/
void CSphere::vGetNormale(int _iFace, int _iVertex, CVector3 *_normale)
{
    int index = 0;
    ...
    _normale->vSet( m_oNormales[index].fGetX(),
                    m_oNormales[index].fGetY(),
                    m_oNormales[index].fGetZ() );
}
...
void CSphere::vSetMaterialColor()
{
